...
class Solution:
    ...
    @classmethod
    def _mark_visited_dfs_recursive(cls, grid, row: int, col: int, islands: int = 0):
        ...
        for δ_row, δ_col in Solution.directions:
            Solution._mark_visited_dfs_recursive(
                grid, row + δ_row, col + δ_col, islands
            )

        return

    @classmethod
    def _mark_visited_dfs_iterative(cls, grid, row: int, col: int, islands: int = 0):
        """Depth-first search method to marks the visited nodes.
        Visited nodes are marked with `In`, where `n` is the island number.
        """
        if (
    ...
    @classmethod
    def _mark_visited_bfs(cls, grid, row: int, col: int, islands: int = 0):
    ...
    def numIslands(
        self, grid: list[list[str]], method: Literal["DFS", "BFS"] = "DFS"
    ) -> int:
        if not grid:
            return 0
        rows = len(grid)
        ...
        strategy = {
            "DFS": Solution._mark_visited_dfs_iterative,
            "BFS": Solution._mark_visited_bfs,
        }
        islands = 0
