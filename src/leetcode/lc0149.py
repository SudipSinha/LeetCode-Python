...
def maxPoints_line_2d(points: list[tuple[int, int]]) -> int:
    ...
    parameters: defaultdict[tuple[Fraction | float, Fraction], int] = defaultdict(int)
    n = len(points)
    for i in range(n - 1):
        for j in range(i + 1, n):
            (slope, intercept) = _get_line_parameters(points[i], points[j])
            parameters[(slope, intercept)] += 1
