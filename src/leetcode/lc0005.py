...
class Solution:
    @classmethod
    def _is_palindrome(cls, s: str) -> bool:
        mid = len(s) / 2
        mid_left = floor(mid)
        mid_right = ceil(mid)
        return s[0:mid_left] == s[: mid_right - 1 : -1]

    def longestPalindrome_naive(self, s: str) -> LP:
        ...
        for substr__len in range(len(s), 0, -1):
            for idx_start in range(0, len(s) - substr__len + 1):
                if Solution._is_palindrome(s[idx_start : idx_start + substr__len]):
    ...
    def longestPalindrome_centerout(self, s: str) -> LP:
    ...
    def longestPalindrome_dp(self, s: str) -> LP:
